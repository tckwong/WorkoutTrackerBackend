...
def post_exercises():
    ...
    try:
        ...
        # First check if exerciseId already exists in DB
        ...
        for index in range(len(duplicates)):
            if (duplicates[index] == True):
                ...
                cnnct_to_db.cursor.execute("INSERT INTO exercise(exercise_name,reps,sets,weight,workout_id,completed,user_id) VALUES(?,?,?,?,?,?,?)",[data[index].get('exerciseName'),data[index].get('reps'),data[index].get('sets'),data[index].get('weight'),data[index].get('workoutId'),data[index].get('completed'),db_userId])
