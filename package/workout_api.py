...
def post_workout():
    ...
    try:
        ...
        cnnct_to_db.cursor.execute("INSERT INTO workout(title,created_on,user_id) VALUES(?,?,?)",[client_title,cur_date,db_userId])
...
#                 elif (key == "completed"):
#                     cur_date = datetime.datetime.now().strftime('%Y-%m-%d')
#                     cnnct_to_db.cursor.execute("UPDATE workout SET completed=? AND completed_on=? WHERE id=?",[cur_date,client_workoutId])
#                 else:
